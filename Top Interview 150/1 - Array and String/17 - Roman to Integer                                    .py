...
class Solution:
    def romanToInt(self, s: str) -> int:
        ...
        for i in range(len(s)):
            # If the current symbol represents a value less than the next symbol
            if i < len(s) - 1 and roman_to_int_map[s[i]] < roman_to_int_map[s[i+1]]:
                # Subtract its value from total_value
                total_value -= roman_to_int_map[s[i]]
                print(
                    f"Subtracting {roman_to_int_map[s[i]]} from total_value: {total_value}")
            else:
                # Add its value to total_value
                total_value += roman_to_int_map[s[i]]
                print(
                    f"Adding {roman_to_int_map[s[i]]} to total_value: {total_value}")
